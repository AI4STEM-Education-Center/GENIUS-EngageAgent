...
const surveyFromItem = (
  ...
  schedule: item.schedule as SurveyRecord["schedule"],
  published_by: item.published_by as string | undefined,
  published_at: item.published_at as string | undefined,
  created_at: (item.created_at as string) ?? "",
  updated_at: (item.updated_at as string) ?? "",
});
      ...
      await client.send(
        new PutCommand({
          ...
          Item: {
            ...
            published_by: survey.published_by,
