...
export async function POST(request: Request) {
  ...
  if (typeof dailyExperienceTopic !== "string" || !dailyExperienceTopic.trim()) {
    return NextResponse.json(
  ...
  // Update the existing survey (preserving created_at) or create a new one.
  ...
  const survey: Survey = {
    ...
    questions:
      (questions as SurveyItem[] | undefined) ?? existing?.questions ?? [],
